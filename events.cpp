...
void events::extendStorage(int ID)
{
    ...
    if(firstTime)
    {
        ...
        stringstream res;
        res << "time\tID\tMID\tcellType\tcellState\tBCL6\tBLIMP1\tIRF4\tEvent\tAffinity\tRecyclings\tnFDCcontacts\tIamAgHigh\tZpos\tZpol"<< endl;
...
{
   ///RRR if(t < 0) t = currentEventsTime;

   ///RRR extendStorage(bc->ID);

  ///RRR  stringstream res2;

  ///RRR  res2 << t << "\t" << bc->ID << "\t" <<bc->MID <<"\t" <<bc->cell_type<<"\t" <<bc->cell_state<<"\t" << bc->BCL6<< "\t" << bc->BLIMP1<< "\t" << bc->IRF4<<"\t"<<nameEvent(typeEvent) << "\t"<< bc->MyAffinity<< "\t" << bc->nRecyclings<< "\t"<< bc->nFDCcontacts<<"\t" <<bc->IamHighAg<<"\t" << bc->position.Z<< "\t" << bc->polarity.Z<<  endl;

  ///RRR  storage[bc->ID]->append(res2.str());

  ///RRR  eventsOut  << *(storage[bc->ID]) << endl;
