...
lattice::lattice(parameters& p, string CXCL12fname, string CXCL13fname)
    : thetas(2), chemo_grid(p) {
  chemo_grid.loadchemokinesfromHyphasma(CXCL12fname, CXCL12);
  chemo_grid.loadchemokinesfromHyphasma(CXCL13fname, CXCL13);
...
cell* lattice::cellat(int x, int y, int z) {
  ...
  if (celltypeat(x, y, z) == celltype::empty) { ///RRR
...
void lattice::putcellat(cell* c) {
  ...
  if (celltypeat(c->position) != celltype::empty)  // Danial: not checked ///RRR
  {
...
vector3D lattice::getFreePosition(double minZperc, double maxZperc) {
  ...
  while ((!foundPlace) && (ocupiedspaces < 10000000)) {
    ...
    if (celltypeat(freePosition) == celltype::empty)  // DAnial: not chekced ///RRR
    {
      foundPlace = true;
      ocupiedspaces = 0;
      return freePosition;
...
vector3D lattice::getfreeNeighbour_nn(vector3D& pos) {
  ...
  for (unsigned int i = 0; i < neighbours_nn.size(); i++) {
    if (insideBorders(neighbours_nn[i])) {
      if (celltypeat(neighbours_nn[i]) == celltype::empty) { ///RRR
...
vector3D lattice::getfreeNeighbour_diag(vector3D& pos) {
  ...
  for (unsigned int i = 0; i < neighbours_diag.size(); i++) {
    if (insideBorders(neighbours_diag[i])) {
      if (celltypeat(neighbours_diag[i]) == celltype::empty) { ///RRR
