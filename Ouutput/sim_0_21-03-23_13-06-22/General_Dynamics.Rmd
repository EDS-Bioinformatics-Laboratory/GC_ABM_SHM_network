...
```{r}
file.name.P <- "Plasma_cells_data.csv"

df.Pcell.time.raw <- read_delim(file.path( file.name.P),  delim = ",", 
                                  col_names = TRUE, col_types = cols(
  ID = col_double(),
  Born_time = col_double(),
  MID = col_double(),
  GMID = col_double(),
  Affinity = col_double(),
  retained_Ag = col_double(),
  N_of_divisions = col_double(),
  N_of_Mutations = col_double(),
  delta_aff = col_double(),
  FDC_interaction_nums = col_double(),
  FDC_interaction_time_total = col_double(),
  TC_interaction_time = col_double()
))
```


```{r}
df.Pcell.time <- df.Pcell.time.raw %>%
    mutate(
    time = round(Born_time )
    )%>%  
  group_by(time)%>%
  mutate(
    n_PCs = n(),
...
```{r}
df.Pcell.time%>%
  ggplot(aes(x = Born_time, y = n_PCs)) +
  geom_point() +
  labs(x = "Time ", y = "PC count") +
...
df.Pcell.time%>%
  ggplot(aes(x = time, y = affinity)) +
  geom_point(size=1) +
  labs(x = "Time (hours) ", y = "PC Affinity") +
...
```{r}
file.name.M <- "Memory_cells_data.csv"

df.Mcell.time.raw <- read_delim(file.path(file.name.M),  delim = ",", 
                                  col_names = TRUE, col_types = cols(
  ID = col_double(),
  Born_time = col_double(),
  MID = col_double(),
...
```{r}
df.Mcell.time <- df.Mcell.time.raw %>%
    mutate(
    time = round(Born_time )
    )%>%  
  group_by(time)%>%
...
```{r}
df.Mcell.time%>%
  ggplot(aes(x = Born_time, y = n_MCs)) +
  geom_point() +
  labs(x = "Time ", y = "MC count") +
...
df.Mcell.time%>%
  ggplot(aes(x = time, y = affinity)) +
  geom_point(size=1) +
  labs(x = "Time (hours) ", y = "MC Affinity") +
  expand_limits(x = 0, y = 0) +
