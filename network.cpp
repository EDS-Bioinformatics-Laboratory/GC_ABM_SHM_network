...
network::network(): odesolver(NbVariables, NbParameters)
{
    ...
    void network::derivatives(const vector<double> &x, vector<double> &dxdt, const double t){
                ...
                dxdt[b] 	= params[Mu_b] + params[sigma_b] * hillI(x[p], params[k_p]) * hillI(x[b], params[k_b]) * hillI(x[r], params[k_r])  - (params[l_b] + (params[bcr] * hillI(x[b], params[k_b]))) * x[b]  ;
